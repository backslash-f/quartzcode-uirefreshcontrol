...
//
//  Code generated using QuartzCode 1.66.2 on 22.03.19.
...
class CustomUIRefreshControl: UIView, CAAnimationDelegate {
	...
	func resetLayerProperties(forLayerIdentifiers layerIds: [String]!){
		...
		if layerIds == nil || layerIds.contains("cloudGradientBackground"){
			...
			cloudGradientBackground.strokeColor   = UIColor(red:0, green: 0.653, blue:0.861, alpha:1).cgColor
			...
			let cloudGradientBackgroundGradientColors : Array <AnyObject> = [UIColor(red:0, green: 0.653, blue:0.861, alpha:1).cgColor, UIColor(red:1.00, green: 1.00, blue:1.00, alpha:1.0).cgColor]
		...
		if layerIds == nil || layerIds.contains("cloudGradientStroke"){
			...
			cloudGradientStroke.lineCap     = .round
		...
		if layerIds == nil || layerIds.contains("cloudUpDownBackground"){
			...
			cloudUpDownBackground.fillColor   = UIColor(red:0, green: 0.653, blue:0.861, alpha:1).cgColor
			cloudUpDownBackground.strokeColor = UIColor(red:0, green: 0.653, blue:0.861, alpha:0).cgColor
		...
		if layerIds == nil || layerIds.contains("cloudUpDownStroke"){
			...
			cloudUpDownStroke.lineCap     = .round
		...
		if layerIds == nil || layerIds.contains("cloudSolidBackground"){
			...
			cloudSolidBackground.fillColor   = UIColor(red:0, green: 0.653, blue:0.861, alpha:1).cgColor
			cloudSolidBackground.strokeColor = UIColor(red:0, green: 0.653, blue:0.861, alpha:0).cgColor
		...
		if layerIds == nil || layerIds.contains("cloudSolidStroke"){
			...
			cloudSolidStroke.lineCap     = .round
		...
		if layerIds == nil || layerIds.contains("cloudStaticBackground"){
			...
			cloudStaticBackground.fillColor   = UIColor(red:0, green: 0.653, blue:0.861, alpha:1).cgColor
			cloudStaticBackground.strokeColor = UIColor(red:0, green: 0.653, blue:0.861, alpha:1).cgColor
		...
		if layerIds == nil || layerIds.contains("cloudStaticStroke"){
			...
			cloudStaticStroke.lineCap     = .round
	...
	func setupLayerFrames(){
		...
		if let cloudGradient = layers["cloudGradient"]{
		...
		if let cloudGradientBackground = layers["cloudGradientBackground"] as? CAShapeLayer{
			...
			cloudGradientBackground.path  = cloudGradientBackgroundPath(bounds: layers["cloudGradientBackground"]!.bounds).cgPath
			let cloudGradientBackgroundGradient = layers["cloudGradientBackgroundGradient"] as! CAGradientLayer
			cloudGradientBackgroundGradient.frame = cloudGradientBackground.bounds
			(cloudGradientBackgroundGradient.mask as! CAShapeLayer).path = cloudGradientBackground.path
		}
		
		if let cloudGradientStroke = layers["cloudGradientStroke"] as? CAShapeLayer{
			...
			cloudGradientStroke.path  = cloudGradientStrokePath(bounds: layers["cloudGradientStroke"]!.bounds).cgPath
		}
		
		if let cloudUpDown = layers["cloudUpDown"]{
		...
		if let cloudUpDownBackground = layers["cloudUpDownBackground"] as? CAShapeLayer{
			...
			cloudUpDownBackground.path  = cloudUpDownBackgroundPath(bounds: layers["cloudUpDownBackground"]!.bounds).cgPath
		}
		
		if let cloudUpDownStroke = layers["cloudUpDownStroke"] as? CAShapeLayer{
			...
			cloudUpDownStroke.path  = cloudUpDownStrokePath(bounds: layers["cloudUpDownStroke"]!.bounds).cgPath
		}
		
		if let cloudSolid = layers["cloudSolid"]{
		...
		if let cloudSolidBackground = layers["cloudSolidBackground"] as? CAShapeLayer{
			...
			cloudSolidBackground.path  = cloudSolidBackgroundPath(bounds: layers["cloudSolidBackground"]!.bounds).cgPath
		}
		
		if let cloudSolidStroke = layers["cloudSolidStroke"] as? CAShapeLayer{
			...
			cloudSolidStroke.path  = cloudSolidStrokePath(bounds: layers["cloudSolidStroke"]!.bounds).cgPath
		}
		
		if let cloudStatic = layers["cloudStatic"]{
		...
		if let cloudStaticBackground = layers["cloudStaticBackground"] as? CAShapeLayer{
			...
			cloudStaticBackground.path  = cloudStaticBackgroundPath(bounds: layers["cloudStaticBackground"]!.bounds).cgPath
		}
		
		if let cloudStaticStroke = layers["cloudStaticStroke"] as? CAShapeLayer{
			...
			cloudStaticStroke.path  = cloudStaticStrokePath(bounds: layers["cloudStaticStroke"]!.bounds).cgPath
		}
	...
	func addRefreshUpDownAnimation(){
        ...
        let cloudUpDown: CALayer = layers["cloudUpDown"]!
	...
	func updateLayerValues(forAnimationId identifier: String){
		if identifier == "refreshGradient"{
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudGradient"]!.animation(forKey: "cloudGradientRefreshGradientAnim"), theLayer:layers["cloudGradient"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudGradientBackground"]!.animation(forKey: "cloudGradientBackgroundRefreshGradientAnim"), theLayer:layers["cloudGradientBackground"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudGradientStroke"]!.animation(forKey: "cloudGradientStrokeRefreshGradientAnim"), theLayer:layers["cloudGradientStroke"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudUpDown"]!.animation(forKey: "cloudUpDownRefreshGradientAnim"), theLayer:layers["cloudUpDown"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudSolid"]!.animation(forKey: "cloudSolidRefreshGradientAnim"), theLayer:layers["cloudSolid"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudStatic"]!.animation(forKey: "cloudStaticRefreshGradientAnim"), theLayer:layers["cloudStatic"]!)
		}
		else if identifier == "refreshSolid"{
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudGradient"]!.animation(forKey: "cloudGradientRefreshSolidAnim"), theLayer:layers["cloudGradient"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudUpDown"]!.animation(forKey: "cloudUpDownRefreshSolidAnim"), theLayer:layers["cloudUpDown"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudSolid"]!.animation(forKey: "cloudSolidRefreshSolidAnim"), theLayer:layers["cloudSolid"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudSolidBackground"]!.animation(forKey: "cloudSolidBackgroundRefreshSolidAnim"), theLayer:layers["cloudSolidBackground"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudSolidStroke"]!.animation(forKey: "cloudSolidStrokeRefreshSolidAnim"), theLayer:layers["cloudSolidStroke"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudStatic"]!.animation(forKey: "cloudStaticRefreshSolidAnim"), theLayer:layers["cloudStatic"]!)
		}
		else if identifier == "refreshUpDown"{
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudGradient"]!.animation(forKey: "cloudGradientRefreshUpDownAnim"), theLayer:layers["cloudGradient"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudUpDown"]!.animation(forKey: "cloudUpDownRefreshUpDownAnim"), theLayer:layers["cloudUpDown"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudUpDownBackground"]!.animation(forKey: "cloudUpDownBackgroundRefreshUpDownAnim"), theLayer:layers["cloudUpDownBackground"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudSolid"]!.animation(forKey: "cloudSolidRefreshUpDownAnim"), theLayer:layers["cloudSolid"]!)
			QCMethod.updateValueFromPresentationLayer(forAnimation: layers["cloudStatic"]!.animation(forKey: "cloudStaticRefreshUpDownAnim"), theLayer:layers["cloudStatic"]!)
		}
	}
	
	func removeAnimations(forAnimationId identifier: String){
		if identifier == "refreshGradient"{
			layers["cloudGradient"]?.removeAnimation(forKey: "cloudGradientRefreshGradientAnim")
			layers["cloudGradientBackground"]?.removeAnimation(forKey: "cloudGradientBackgroundRefreshGradientAnim")
			layers["cloudGradientStroke"]?.removeAnimation(forKey: "cloudGradientStrokeRefreshGradientAnim")
			layers["cloudUpDown"]?.removeAnimation(forKey: "cloudUpDownRefreshGradientAnim")
			layers["cloudSolid"]?.removeAnimation(forKey: "cloudSolidRefreshGradientAnim")
			layers["cloudStatic"]?.removeAnimation(forKey: "cloudStaticRefreshGradientAnim")
		}
		else if identifier == "refreshSolid"{
			layers["cloudGradient"]?.removeAnimation(forKey: "cloudGradientRefreshSolidAnim")
			layers["cloudUpDown"]?.removeAnimation(forKey: "cloudUpDownRefreshSolidAnim")
			layers["cloudSolid"]?.removeAnimation(forKey: "cloudSolidRefreshSolidAnim")
			layers["cloudSolidBackground"]?.removeAnimation(forKey: "cloudSolidBackgroundRefreshSolidAnim")
			layers["cloudSolidStroke"]?.removeAnimation(forKey: "cloudSolidStrokeRefreshSolidAnim")
			layers["cloudStatic"]?.removeAnimation(forKey: "cloudStaticRefreshSolidAnim")
		}
		else if identifier == "refreshUpDown"{
			layers["cloudGradient"]?.removeAnimation(forKey: "cloudGradientRefreshUpDownAnim")
			layers["cloudUpDown"]?.removeAnimation(forKey: "cloudUpDownRefreshUpDownAnim")
			layers["cloudUpDownBackground"]?.removeAnimation(forKey: "cloudUpDownBackgroundRefreshUpDownAnim")
			layers["cloudSolid"]?.removeAnimation(forKey: "cloudSolidRefreshUpDownAnim")
			layers["cloudStatic"]?.removeAnimation(forKey: "cloudStaticRefreshUpDownAnim")
		}
	}
